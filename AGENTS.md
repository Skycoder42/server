...
- Dev tools (`ruff`, `mypy`, `django-stubs`) come from `requirements-dev.txt`. Caveat: it pins `django==5.0.6` while `requirements.txt` pins `django==4.2.14`, so installing both in one pip command conflicts. `requirements.in/{base,development}.txt` are the pip-compile sources; regenerate with `pip-compile` if you touch them.
...
- Responses and routes use `MsgpackResponse` / `MsgpackRoute` (`fastapi/msgpack.py`), not plain JSON. Errors go through `CustomHttpException`/`HttpError`.
- The `/api/v1/ws/*` websocket endpoints require Redis (`ETEBASE_REDIS_URI`); they raise `NotSupported` without it.
...
- Style: ruff + black, line-length 120, double quotes; ruff excludes `migrations/` dirs. Run `ruff check .` and `mypy .` (mypy uses the django-stubs plugin via `mypy.ini`).
